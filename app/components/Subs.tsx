...
export default function UsersSub() {
  ...
  const [editPeriodicity, setEditPeriodicity] = React.useState("");
  const token = localStorage.getItem("token");
  ...
  const handleEdit = (id, amount, periodicity) => {
    setEditingId(id);
    setEditAmount(amount);
    setEditPeriodicity(periodicity);
  };
  ...
  const handleSave = async (id) => {
    try {
      ...
      const updatedSubscriptions = subscriptions.map((subscription) =>
        subscription.subscriptionplanid === id
          ? { ...subscription, amount: editAmount, periodicity: editPeriodicity }
          : subscription
      );
      setSubscriptions(updatedSubscriptions);
      setEditingId(null);
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right">
                      {editingId === row.subscriptionplanid ? (
                        <TextField
                          value={editAmount}
                    ...
                    <TableCell>
                      {editingId === row.subscriptionplanid ? (
                        <Button
                          variant="contained"
                          color="primary"
                          onClick={() => handleSave(row.subscriptionplanid)}
                        >
                          Save
                        </Button>
                      ) : (
                        <Button
                          variant="contained"
                          color="secondary"
                          onClick={() => handleEdit(row.subscriptionplanid, row.amount, row.periodicity)}
                        >
                          Edit
                        </Button>
                      )}
